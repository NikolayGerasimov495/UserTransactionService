...
class PydanticModelTransformer:
    ...
    def add_initializer(self, fields: List['PydanticModelField'], config: 'ModelConfigData', is_settings: bool) -> None:
        """
        Adds a fields-aware `__init__.py` method to the class.

        The added `__init__.py` will be annotated with types vs. all `Any` depending on the plugin settings.
        """
        ctx = self._ctx
        ...
        add_method(ctx, '__init__.py', init_arguments, NoneType())

    def add_construct_method(self, fields: List['PydanticModelField']) -> None:
        """
        Adds a fully typed `construct` classmethod to the class.

        Similar to the fields-aware __init__.py method, but always uses the field names (not aliases),
    ...
    def get_field_arguments(
        ...
        """
        Helper function used during the construction of the `__init__.py` and `construct` method signatures.
    ...
    def should_init_forbid_extra(self, fields: List['PydanticModelField'], config: 'ModelConfigData') -> bool:
        """
        Indicates whether the generated `__init__.py` should get a `**kwargs` at the end of its signature
