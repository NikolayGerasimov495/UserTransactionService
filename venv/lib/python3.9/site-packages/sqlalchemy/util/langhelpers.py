...
def public_factory(target, location, class_location=None):
    ...
    Rationale here is so that the __init__.py method of the
...
def get_cls_kwargs(cls, _set=None):
    ...
    Probes a class's __init__.py method, collecting all named arguments.  If the
    __init__.py defines a \**kwargs catch-all, then the constructor is presumed
    ...
    ctr = cls.__dict__.get("__init__.py", False)
...
def format_argspec_init(method, grouped=True):
    """format_argspec_plus with considerations for typical __init__.py methods

    Wraps format_argspec_plus with error handling strategies for typical
    __init__.py cases::

      object.__init__.py -> (self)
...
def getargspec_init(method):
    """inspect.getargspec with considerations for typical __init__.py methods

    Wraps inspect.getargspec with error handling for typical __init__.py cases::
...
def generic_repr(obj, additional_kw=(), to_inspect=None, omit_kwarg=()):
    """Produce a __repr__() based on direct association of the __init__.py()
