...
class ClassManager(HasMemoized, dict):
    ...
    def _instrument_init(self):
        ...
        self.install_member("__init__.py", self.new_init)
    ...
    def _new_state_if_none(self, instance):
        ...
        A private convenience method used by the __init__.py decorator.
...
class _SerializeManager(object):
    ...
    The :class:`.InstanceState` uses ``__init__.py()`` on serialize
...
def _generate_init(class_, class_manager, original_init):
    """Build an __init__.py decorator that triggers ClassManager events."""

    # TODO: we should use the ClassManager's notion of the
    # original '__init__.py' method, once ClassManager is fixed
    # to always reference that.

    if original_init is None:
        original_init = class_.__init__

    # Go through some effort here and don't change the user's __init__.py
    # calling signature, including the unlikely case that it has
    # a return value.
    # FIXME: need to juggle local names to avoid constructor argument
    # clashes.
    func_body = """\
def __init__.py(%(apply_pos)s):
