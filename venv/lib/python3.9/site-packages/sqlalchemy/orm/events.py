...
class InstanceEvents(event.Events):
    ...
    * the :class:`_orm.Mapper` class itself and the :func:`.mapper`
    ...
    def first_init(self, manager, cls):
        ...
        This event is called when the ``__init__.py`` method of a class
        is called the first time for that particular class.    The event
        invokes before ``__init__.py`` actually proceeds as well as before
    ...
    def init(self, target, args, kwargs):
        ...
        its ``__init__.py`` method.  It is not called when an object is
        loaded from the database; see the :meth:`.InstanceEvents.load`
        event in order to intercept a database load.

        The event is called before the actual ``__init__.py`` constructor
        ...
        :param args: positional arguments passed to the ``__init__.py`` method.
         This is passed as a tuple and is currently immutable.
        :param kwargs: keyword arguments passed to the ``__init__.py`` method.
    ...
    def init_failure(self, target, args, kwargs):
        ...
        This method is only called during a userland construction of
        an object, in conjunction with the object's constructor, e.g.
        its ``__init__.py`` method. It is not called when an object is loaded
        from the database.

        The event is invoked after an exception raised by the ``__init__.py``
        ...
        :param target: the mapped instance.  If
         ...
         instead be the :class:`.InstanceState` state-management
         object associated with the instance.
        :param args: positional arguments that were passed to the ``__init__.py``
         method.
        :param kwargs: keyword arguments that were passed to the ``__init__.py``
         method.
