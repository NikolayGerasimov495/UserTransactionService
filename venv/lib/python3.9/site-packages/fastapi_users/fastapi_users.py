...
class FastAPIUsers(Generic[models.UP, models.ID]):
    ...
    def get_auth_router(
        ...
        """
        logger.info(f'Создание маршрутизатора аутентификации с бэкендом: {backend.name}, '
                    f'требуется верификация: {requires_verification}')
    ...
    def get_users_router(
        ...
        """
        logger.info(f'Создание маршрутизатора пользователей с схемой: {user_schema.__name__}, '
                    f'схемой обновления: {user_update_schema.__name__}, '
                    f'требуется верификация: {requires_verification}')
